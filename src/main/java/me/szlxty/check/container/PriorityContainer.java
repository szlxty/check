...
import me.szlxty.check.Ordered;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * <p>对Error可进行优先级别管理的错误容器。优先级高的错误描述将替换优先级低的错误描述。</p>
 * <p>一般的，错误可以通过实现接口{@code me.szlx.validation.Ordered}表明自身的优先级，序号越大，则优先级越高。</p>
 * <p>没有实现接口{@code me.szlx.validation.Ordered}时，默认序号为0。</p>
...
public class PriorityContainer extends AbstractContainer {
    ...
    protected void doInstall(Constraint constraint) {
        if (constraint != null) {
            ...
            } else if (Ordered.DESC.compare(constraint, ruleExisted) != -1) {
                getErrorMap().put(constraint.code(), constraint);
